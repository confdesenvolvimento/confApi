...
@Component
public class ChatIaHotelListaClient {
    ...
    public ChatIaHotelListaClient(ChatConfiancaTokenProvider auth,ObjectMapper mapper) {
        this(auth,mapper,()->UrlConfig.URL_CONFIANCA_MANAGER,new OkHttpClient());
    }
    ChatIaHotelListaClient(ChatConfiancaTokenProvider auth,ObjectMapper mapper,java.util.function.Supplier<String> baseUrl,OkHttpClient transport) {
    ...
    public ChatIaHotelListaDocumento consultar(ChatIaHotelListaDocumento.Consulta consulta,int timeout) throws IOException {
        return request(consulta,timeout,ChatIaHotelListaDocumento.class);
    }
    private <T> T request(ChatIaHotelListaDocumento.Consulta q,int timeout,Class<T> type) throws IOException {
        String base=baseUrl.get(),token=auth.bearerToken();
        if(base==null||base.isBlank()||token==null||token.isBlank())throw new IOException("HOTEL_LISTA_CONEXAO_AUSENTE");
        Request.Builder r=new Request.Builder().url(base.replaceAll("/+$","")+"/chatIa/runtime/hotel-lista/"+(q==null?"configuracao":"consulta"))
        ...
        try(Response response=http.newCall(r.build()).execute()) {
            ...
            if(response.code()!=200||response.body()==null)throw new IOException("HOTEL_LISTA_HTTP_"+response.code());
            byte[] bytes=response.body().byteStream().readNBytes(262145);
            if(bytes.length>262144)throw new IOException("HOTEL_LISTA_RESPOSTA_EXCESSIVA");

...
public class ChatV2Planner {
    ...
    static ChatV2Capability identificar(String t) {
        ...
        if(t.matches(".*\\b(localizador|abrir reserva|detalhes da reserva)\\b.*"))return ChatV2Capability.RESERVA;
        if(t.matches(".*\\b(familia|familias|bagagem)\\b.*"))return ChatV2Capability.FAMILIAS;
        if(t.matches(".*\\b(check[ -]?ins?|embarques)\\b.*")
                && !t.matches(".*\\b(hotel|hoteis|hospedagem)\\b.*"))return ChatV2Capability.CHECKIN;
        if(t.matches(".*\\b(alerta|alertas)\\b.*"))return ChatV2Capability.ALERTAS;
        if(t.matches(".*\\b(hotel|hoteis|hospedagem)\\b.*"))return ChatV2Capability.HOTEL;
    ...
    static boolean continuacao(String t,ChatV2Plan p) {
        return t.matches("^(e |sim\\b|nao\\b|cade\\b|como pego|onde baixar|me mande|me mostre as tarifas|pesquise entao|emissao |\\d).*")
                || (reservaComLocalizador(p)&&t.matches("[a-z0-9]{6}"))
    ...
    private static boolean comandoInequivoco(String t,ChatV2Plan p) {
        ...
        if(p.isContinuar()&&reservaComLocalizador(p)&&t.matches("[a-z0-9]{6}"))return true;
    ...
    static void extrairLocais(ChatV2Plan p,String mensagem) {
        String t=normalizar(mensagem);
        Matcher loc=Pattern.compile("(?i)\\b(?:localizador|loc|reserva)\\s+([a-z0-9]{6})\\b").matcher(mensagem);
        if(loc.find())p.getParametros().put("localizador",loc.group(1).toUpperCase(Locale.ROOT));
        else if(mensagem.trim().matches("[A-Za-z0-9]{6}"))p.getParametros().put("localizador",mensagem.trim().toUpperCase(Locale.ROOT));
        ...
        for(int i=historico.size()-1;i>=Math.max(0,historico.size()-10);i--) {
            ...
            try {
                ...
                for(JsonNode action:payload.path("actions")) {
                    ...
                    if(localizador.matches("[A-Za-z0-9]{6}"))p.getParametros().put("localizador",localizador.toUpperCase(Locale.ROOT));

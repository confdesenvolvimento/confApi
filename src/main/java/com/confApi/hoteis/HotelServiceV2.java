...
import com.confApi.hoteis.dto.ReservaHotelCancelamentoDto;
...
public class HotelServiceV2 {
    ...
    public HotelServiceV2(RestTemplate restTemplate) {
    ...
    public ReservaHotel criarReservaDB(ReservaHotel reservaHotel) {
        try {
            ConfAppResp token = confAppService.token();

            HttpHeaders headers = defaultHeaders(token.getToken());
            headers.setContentType(MediaType.APPLICATION_JSON);
            HttpEntity<ReservaHotel> entity = new HttpEntity<>(reservaHotel, headers);

            ResponseEntity<ReservaHotel> response =
                    restTemplate.exchange(
                            UrlConfig.URL_CONFIANCA_MANAGER + "/reservaHotel",
                            HttpMethod.POST,
                            entity,
                            ReservaHotel.class
                    );
            ...
            if (!response.getStatusCode().is2xxSuccessful()) {
                throw new RuntimeException("Erro ao chamar HUB Reserva Hotel. HTTP " + response.getStatusCode());
            }
    ...
    public void cancelarReservaDB(ReservaHotelModel req) {
        ...
        try {
            ...
            HttpHeaders headers = defaultHeaders(token.getToken());
            headers.setContentType(MediaType.APPLICATION_JSON);
            HttpEntity<ReservaHotelCancelamentoDto> entity = new HttpEntity<>(reservaHotelCancelamentoDto, headers);

            ResponseEntity<String> response =
                    restTemplate.exchange(
                            UrlConfig.URL_CONFIANCA_MANAGER + "/reservaHotel/cancelar/" + req.getCodgHotel(),
                            HttpMethod.PUT,
                            entity,
                            String.class
                    );
